package eightbitsakabigbyte.Entity;
import eightbitsakabigbyte.Service.*;


/**
 * Created by jeriahhumphrey on 3/8/17.
 */
public class GameLogic {


    public boolean isLegalMove(GameBoard board, MoveRequest request) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                if (board.getGameBoard()[i][j].getIdentifier() == request.getId()) {
                    if (board.getGameBoard()[request.getRow()][request.getColumn()].getIdentifier() < 0){
                        if (i - request.getRow() == 1 && (j - request.getColumn() == 1 || j - request.getColumn() == -1)) {
                            return true;
                        }
                    }
                }
            }
        }
        return false;
    }

}